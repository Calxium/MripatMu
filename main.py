...
def check_internet(host="8.8.8.8", port=53, timeout=3):
    """
    Check if there is an internet connection by trying to reach a DNS server (Google's public DNS).
    """
    try:
    ...
    if faceCurFrame:
        for encodeFace, faceLoc in zip(encodeCurFrame, faceCurFrame):
            ...
            if matches[matchIndex]:
                ...
                if counter == 0:
                    cvzone.putTextRect(imgBackground, "Loading", (275, 400),scale=3, thickness=2, offset=10, colorR=(0, 255, 255))
        ...
        if counter != 0:
            if check_internet():
                if counter == 1:
                    ...
                    datetimeObject = datetime.strptime(studentInfo['last_out_time'], "%Y-%m-%d %H:%M:%S")
                    ...
                    if secondsElapsed > 30:
                        ...
                        ref.child('last_out_time').set(datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
                    else:
                        modeType = 3
                        counter = 0
                        imgBackground[44:44 + 633, 808:808 + 414] = imgModeList[modeType]
                ...
                if modeType != 3:
                    ...
                    if counter <= 10:
                        cv2.putText(imgBackground, str(studentInfo['total_out']), (861, 125), cv2.FONT_HERSHEY_COMPLEX,
                                    1, (255, 255, 255), 1)
                ...
                cvzone.putTextRect(imgBackground, "No Internet", (275, 400), scale=3, thickness=2, offset=10,
                                   colorR=(0, 0, 255))
                counter = 0
                modeType = 0
